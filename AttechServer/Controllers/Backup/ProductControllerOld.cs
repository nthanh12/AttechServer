...
namespace AttechServer.Controllers
{
    ...
    public class ProductController : ApiControllerBase
    {
        ...
        public async Task<ApiResponse> FindAll([FromQuery] PagingRequestBaseDto input)
        {
            try
            {
                return new(ApiStatusCode.Success, await _productService.FindAll(input), 200, "Ok");
            }
            catch (Exception ex)
            {
        ...
        {
            try {
                return new(ApiStatusCode.Success, await _productService.FindAllByCategorySlug(input, slug), 200, "Ok");
            }
            catch (Exception ex)
            {
                return OkException(ex);
        ...
        public async Task<ApiResponse> FindById(int id)
        {
            try
            {
                return new(ApiStatusCode.Success, await _productService.FindById(id), 200, "Ok");
            }
            catch (Exception ex)
            {
                return OkException(ex);
        ...
        public async Task<ApiResponse> FindBySlug(string slug)
        {
            try
            {
                return new(ApiStatusCode.Success, await _productService.FindBySlug(slug), 200, "Ok");
            }
            catch (Exception ex)
            {
                return OkException(ex);
